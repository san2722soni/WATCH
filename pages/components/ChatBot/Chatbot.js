...
const Chatbot = () => {
	const arrOfMessages = [
		...
		"Yeah Of-Course",
		"CHere's my room id: ",
		"CYeah it's copied to clipboard",
		"Let's go...",
	];
	...
	const fireName = async () => {
		...
		if (name) {
			let confirmation = (await Swal.fire(`Entered name: ${name}`))
				.isConfirmed;
	...
	return (
		<>
			<ToastContainer
				...
				theme="light"
			/>
			<div className="mx-10 mt-10 h-[25rem] w-1/4 cursor-pointer rounded-md bg-[#202020] shadow-white-glow shadow-white/30 transition-all">
				<p
					onClick={fireName}
					className="m-3 inline-block rounded-sm bg-[#E4E4E4] p-3 text-sm text-[#202020]"
					id=""
				>
					INVICTUS - Bot here !
				</p>

				<div className="flex flex-col " id="Replies"></div>
			</div>
		</>
	);
};
